...
def cf_stripe_events(cloud_event):
    ...
    # Find all .parquet files in the current folder
    ...
    # Determine destination bucket based on topic
    ...
    for i, blob in enumerate(parquet_blobs, start=1):
        ...
        try:
            ...
            load_job.result()
            print(f"Successfully loaded file {i} into BigQuery")
            ...
            blob.delete()
            print(f"Moved file to: gs://{processed_bucket_name}/{dest_blob_name}")

        except GoogleAPIError as e:
            print(f"Google API error while processing file {i}: {e.message}")
        except Exception as e:
            print(f"Unexpected error while processing file {i}: {str(e)}")

    print("All Stripe event files have been processed and moved.")
